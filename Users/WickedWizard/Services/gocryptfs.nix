...
in
{
  ...
  systemd.user.services.gocryptfs = {
    Service = {
      ...
      Restart = "on-failure";
      RestartSec = 5;
      ExecStartPre = script "gocryptfs-premount.sh"
        (map_cmd (n: v: "mkdir -p ${v.mountpoint}"))
        [ ]
        [ ]
        { };

      ExecStart = script "gocryptfs-mount.sh"
        (map_cmd (n: v: "gocryptfs ${v.source} ${v.mountpoint} ${lib.optionalString (v ? authentication && v.authentication) "-passfile ${config.age.secrets.${n}.path}"}"))
        [ pkgs.gocryptfs ]
        [ "cannot:${lib.getExe' pkgs.gocryptfs "gocryptfs"}" ]
        { };

      ExecStop = script "gocryptfs-unmount.sh"
        (map_cmd (n: v: "fusermount -u ${v.mountpoint}"))
        [ pkgs.fuse ]
        [ ]
        { external = [ "fusermount" ]; };
