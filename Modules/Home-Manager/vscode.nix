...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    home.activation.vscodeProfiles = hm.dag.entryAfter [ "writeBoundary" ] (
      let
        modifyGlobalStorage = pkgs.writeShellScript "vscode-global-storage-modify" ''
          ...
          if [ -f "$file" ]; then
            ...
            file_write=""
            profiles=(${escapeShellArgs (flatten (mapAttrsToList (n: v: n) allProfilesExceptDefault))})
    ...
    home.file = mkMerge (flatten [
      (mapAttrsToList (n: v: [
        ...
        (mkIf (v.globalSnippets != { }) {
          "${snippetDir n}/global.code-snippets".source = jsonFormat.generate "user-snippet-global.code-snippets" v.globalSnippets;
        })
      ]) cfg.profiles)
      ...
      (mkIf (cfg.profiles != { }) (
        ...
        if (cfg.mutableExtensionsDir && allProfilesExceptDefault == { }) then
          ...
          mkMerge (
            concatMap toPaths (flatten (mapAttrsToList (n: v: v.extensions) cfg.profiles))
            ++ optional (versionAtLeast vscodeVersion "1.74.0" && defaultProfile != { }) {
