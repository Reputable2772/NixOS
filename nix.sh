...
ci() {
	...
	for pc in $(nix flake show --json | jq '.nixosConfigurations | keys[]'); do
		nix-fast-build --skip-cached --no-nom --flake .#nixosConfigurations."$pc".config.system.build.toplevel | cachix push spearman4157
	done

	nix build .#cachix-deploy --out-link deploy.json
